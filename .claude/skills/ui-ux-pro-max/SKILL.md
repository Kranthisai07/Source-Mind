---
name: ui-ux-pro-max
description: "UI/UX design intelligence for the SourceMind React frontend. Use when designing, building, reviewing or fixing interfaces: pages, components, design systems, accessibility, interaction, responsive layout, typography, colour, charts and stack-specific implementation. Searchable local data: 79 styles (50 active), 192 product palettes and reasoning profiles, 74 font pairings, 119 UX guidelines, 105 icons, 17 GSAP presets, 25 chart types, and 3 vendored stacks (react, shadcn, html-tailwind)."
---
...
## This repository's copy — read first

Vendored from github.com/nextlevelbuilder/ui-ux-pro-max-skill (MIT, see
`LICENSE`) because `/plugin marketplace add` is unavailable in this
environment. Runtime files only.

**Running it.** All commands below are relative to the repository root and need
no virtualenv - the scripts import only the standard library plus their own
siblings:
...
Verified working on Python 3.11 and 3.14 from the repo root.

**What was left out, and what that costs.** Domains and stacks load lazily, so a
missing file breaks only its own lookup and reports
`Error: Stack file not found: <path>` rather than crashing.

| Omitted | Size | Effect |
|---------|------|--------|
| 19 of 22 stack CSVs | ~340 KB | `--stack` works only for `react`, `shadcn`, `html-tailwind` |
| `scripts/tests/**`, `validate_data.py` | ~330 KB | upstream's own test and catalog-maintenance tooling |
| `phosphor-icons-upstream.json` | 824 KB | none - no runtime reference in the four scripts |
| `google-font-licenses.json` | 433 KB | none - no runtime reference |
| `data-provenance.json`, `catalog-summary.json` | 39 KB | none - no runtime reference |

To restore a stack, drop its CSV into `data/stacks/` from the upstream repo.
`google-fonts.csv` (747 KB) IS included, because `core.py` registers it as
the `google-fonts` domain and omitting it would make that domain fail.

**The frontend this skill is for.** React 19 + react-router-dom 7, built by
Create React App 5 through CRACO - not Next.js, not Vite. Tailwind CSS 3 with
Radix primitives in the shadcn arrangement (`src/components/ui/`, 47
components), Recharts for charts, `lucide-react` for icons. So
`--stack react` and `--stack shadcn` are the relevant ones;
`--stack nextjs` guidance does not apply here and was not vendored.

**A design system already exists. Do not overwrite it.** `--design-system`
proposes a palette from scratch, which will not match what is built. The
committed tokens live in `apps/web/tailwind.config.js` under
`colors.sm`, alongside the shadcn HSL variables in
`apps/web/src/index.css`:

| Token | Value | | Token | Value |
|-------|-------|-|-------|-------|
| `sm-bg` | `#0A0A0F` | | `sm-blue` | `#4F7EFF` |
| `sm-surface` | `#12121A` | | `sm-purple` | `#A78BFA` |
| `sm-border` | `#1E1E2E` | | `sm-green` | `#34D399` |
| `sm-text` | `#E8E8F0` | | `sm-amber` | `#F59E0B` |
| `sm-text-secondary` | `#8888A8` | | `sm-red` | `#EF4444` |

The app is dark-only. Treat `--design-system` output as a second opinion to
check the existing palette against - contrast ratios, semantic role coverage,
chart colours - not as a replacement. Use the `style`, `ux`,
`color` and `chart` domains for targeted questions instead; those
compose with what is already there.
